...
public class SQLParserOfDB2WithUpdateTest {
    ...
    private static final Object[] UPDATE_ARGS   = new Object[] { 100, "zhouxiaoqing", "xuanxuan",
            13                                 };

    private static final String   UPDATE_NOBIND = "update users set c1 = 100, c2 = 'nihao' where c3 = 'zhouxiaoqing' and  c4='xuanxuan' AND ROWNUM<=13";
    ...
    public void testParseWithPartination() {
        ...
        SqlParserResult parserResult = sqlParser.parse(UPDATE, DBType.DB2);
        ...
        //        Assert.assertEquals(13, parserResult.getMax(Arrays.asList(UPDATE_ARGS)));
        ...
        Map<String, Comparative> patitions = parserResult.getComparativeMapChoicer().getColumnsMap(
            Arrays.asList(UPDATE_ARGS), partinationSet);
    ...
    public void testParserWithNoBindPartination() {
        SQLParser sqlParser = new DefaultSQLParser();
        SqlParserResult parserResult = sqlParser.parse(UPDATE_NOBIND, DBType.DB2);
        Assert.assertEquals("users", parserResult.getTableName());
        Assert.assertEquals(true, parserResult.getGroupByEles().isEmpty());
        Assert.assertEquals(GroupFunctionType.NORMAL, parserResult.getGroupFuncType());
